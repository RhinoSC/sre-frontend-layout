...
nodecg.listenFor('timerStart', (data, ack: any) => {
  startTimer()
    .then(() => ack(null))
    .catch((err) => ack(err));
});
nodecg.listenFor('timerPause', (data, ack: any) => {
  pauseTimer()
    .then(() => ack(null))
    .catch((err) => ack(err));
});
nodecg.listenFor('timerReset', (force, ack: any) => {
  resetTimer(force)
    .then(() => ack(null))
    .catch((err) => ack(err));
});
nodecg.listenFor('timerStop', (data, ack: any) => {
  let myData = data
  stopTimer(data.id, data.forfeit)
    .then(() => ack(null))
    .then(() => {
      nodecg.sendMessage('runnerEnded', data)
    })
    // .then(() => {
    //     nodecg.sendMessage('runnerEnded', {
    //         id: data.id,
...
nodecg.listenFor('timerUndo', (id, ack: any) => {
  undoTimer(id)
    .then(() => ack(null))
    .catch((err) => ack(err));
});
nodecg.listenFor('timerEdit', (time, ack: any) => {
  editTimer(time)
    .then(() => ack(null))
    .catch((err) => ack(err));
});

nodecg.log.info('[Timer] loaded extension')
