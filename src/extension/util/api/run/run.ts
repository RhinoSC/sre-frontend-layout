...
export const apiUpdateRun = async <T>(run: Run): Promise<APIResponse<T>> => {
  try {
    const response = await api.patch(`/runs/${run.id}`, run)
    const apiResponse: APIResponse = response.data
    return apiResponse
  } catch (error) {
...
export const apiUpdateScheduleRuns = async <T>(runs: Run[]): Promise<APIResponse<T>> => {
  try {
    const response = await api.post("/runs/order", runs)
    const apiResponse: APIResponse = response.data
    return apiResponse
  } catch (error) {
...
export const apiGetTwitchCategories = async <T>(name: string): Promise<APIResponse<T>> => {
  try {
    const response = await api.get(`/runs/twitch/categories?name=${name}`)
    const apiResponse: APIResponse = response.data
    return apiResponse
  } catch (error) {
...
export const apiGetTwitchCategoryByID = async <T>(id: string): Promise<APIResponse<T>> => {
  try {
    const response = await api.get(`/runs/twitch/game?id=${id}`)
    const apiResponse: APIResponse = response.data
    return apiResponse
  } catch (error) {
    throw new Error("Failed to get categories");
